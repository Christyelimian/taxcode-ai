/**
 * Script to publish the New Tax Act insight about bank account funds using Firebase directly
 * Run with: npx tsx publish-insight-direct.ts
 */
...
import { getFirestore as getFirestoreClient } from 'firebase-admin/firestore';
...
In simple terms, the New Tax Act is not about taxing savings, but about closing gaps between income, records and compliance. Those who keep proper records and file correctly have little to fear, while the focus is on transparency, fairness and self-compliance rather than punishment.`,
    ...
    const existingQuery = insightsRef.where('slug', '==', insightData.slug);
    const existingDocs = await existingQuery.get();

    if (!existingDocs.empty) {
      console.log('⏭️  Insight already exists with slug:', insightData.slug);
      const existingDoc = existingDocs.docs[0];
      console.log('📄 Existing title:', existingDoc.data().title);
      console.log('🔗 Existing slug:', existingDoc.data().slug);
      console.log('🆔 Existing ID:', existingDoc.id);
      return;
    }
    ...
    const docRef = await insightsRef.add(insightData);

    console.log('✅ Insight published successfully!');
    console.log('🆔 ID:', docRef.id);
    console.log('📄 Title:', insightData.title);
    console.log('🔗 Slug:', insightData.slug);
    console.log('📅 Published at:', insightData.publishedAt.toISOString());
    console.log('🏷️  Tags:', insightData.tags.join(', '));
    console.log('⭐ Featured:', insightData.isFeatured ? 'Yes' : 'No');
    console.log('📖 Published:', insightData.isPublished ? 'Yes' : 'No');

    console.log('\n🔗 You can view the insight at:');
    console.log(`   /insights/${insightData.slug}`);
