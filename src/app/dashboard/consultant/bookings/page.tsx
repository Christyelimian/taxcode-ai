...
import { Mail, Phone, Clock, User, LoaderCircle } from "lucide-react";
...
export default function ConsultantBookingsPage() {
  ...
  useEffect(() => {
    async function load() {
      setIsLoading(true);
      try {
        // Get consultant ID from /api/consultant/me
        ...
        if (data.success && data.data?.id) {
          ...
          if (bookingsResult.success && bookingsResult.data) {
            setBookings(bookingsResult.data as BookingRequest[]);
          }
        }
      } catch (error) {
        toast({
    ...
    <div className="p-4 sm:p-6 md:p-8">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-1">Manage consultation requests from clients</p>
        </div>
            ...
            {bookings.map((booking) => (
              <Card key={booking.id}>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    <p className="text-sm font-semibold mb-1">Request Summary</p>
