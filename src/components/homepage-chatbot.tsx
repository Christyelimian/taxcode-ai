...
export default function HomepageChatbot() {
    ...
    const handleSubmit = async (e?: FormEvent<HTMLFormElement>, quickQuestion?: string) => {
        ...
        const response = await getAiResponse({ question: currentInput });
    ...
    return (
        <>
            <div className={cn("fixed bottom-0 right-0 m-6 z-50 transition-transform duration-300", isOpen ? "translate-x-[500px]" : "translate-x-0")}>
                            ...
                            <div className="p-4 space-y-4">
                               {messages.map((message) => (
                                    <div key={message.id} className={cn('flex items-start gap-3', message.role === 'user' && 'justify-end')}>
                                        ...
                                        <div className={cn('max-w-xs rounded-lg p-3 text-sm shadow', message.role === 'user' ? 'bg-primary text-primary-foreground rounded-br-none' : 'bg-muted rounded-bl-none')}>
